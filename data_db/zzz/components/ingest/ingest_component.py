...
class BaseIngestComponentWithIndex(BaseIngestComponent, abc.ABC):
    ...
    def _initialize_index(self) -> BaseIndex[IndexDict]:
        ...
        try:
            ...
            index.storage_context.persist(persist_dir=AppSettings.paths.persist_dir)   # persist_dir
        return index

    def _save_index(self) -> None:
        self._index.storage_context.persist(persist_dir=AppSettings.paths.persist_dir)  # persist_dir
...
class PipelineIngestComponent(BaseIngestComponentWithIndex):
    ...
    def bulk_ingest(self, files: list[tuple[str, Path]]) -> list[Document]:
        docs = []
        for file_name, file_data in tqdm(files):
            try:
                documents = IngestionHelper.transform_file_into_documents(
                    file_name, file_data
                )
