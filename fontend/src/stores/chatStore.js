...
export const useChatStore = create()(persist((set, get) => ({
    ...
    updateSessionTitle(id, newTitle) {
        set((state) => ({
            conversations: state.conversations.map((conv) => conv.id === id ? { ...conv, title: newTitle } : conv),
        }));
    },
    // 为指定会话自动生成标题
    autoGenerateTitle(conversationId) {
        set((state) => {
            ...
            if (firstUserMessage) {
                ...
                return {
                    conversations: state.conversations.map(c => c.id === conversationId ? { ...c, title: newTitle } : c)
                };
            }
